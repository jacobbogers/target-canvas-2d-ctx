...
describe.skip('Canvas-base', () => {
	it('set & get the canvas width', () => {
		...
		expect(advance).toEqual({
			offsetForArguments: 3,
			offsetForReturnArguments: 3,
		});
	...
	it('getContext (using happy-dom)', () => {
		...
		expect(advance).toEqual({
			offsetForArguments: 4,
			offsetForReturnArguments: 1,
		});
	...
	it('toDataURL (using canvas)', () => {
		...
		const lengthDescr = Math.ceil(
			Math.ceil(Math.log2(imagePNG.byteLength)) / 8,
		);
		const byteLength =
			1 /*for type*/ +
