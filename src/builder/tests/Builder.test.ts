...
describe('Builder', () => {
	it('integers', () => {
		...
		expect(target).toEqual(
			new Uint8Array([
				...
				254, // -2
				33,
				128, // - 128 because the range 0xff80-0xff can be encoded in 1 byte
				34,
				127,
				255, // 0xff7f -129
				34,
				...
				255,
				127,
				255, // 0xff7fff = -32769 (range 0xff8000 - 0xffffff is already covered by 2 byte encoding sequence)
				38,
				255,
				255,
				255,
	...
	it('booleans', () => {
		...
		expect(target).toEqual(new Uint8Array([0x31, 0x30]));
	});
	it('string', () => {
		const builder = createBuilder();
		...
		expect(target).toEqual(
			Uint8Array.from([
				17, 4, 116, 114, 117, 101, 17, 5, 102, 97, 108, 115, 101, 16,
			]),
		);
	});
	it('ubytes', () => {
	...
	describe('oid', () => {
		it('oid no payload', () => {
			...
			expect(advance).toEqual({
				offsetForArguments: 11,
				offsetForReturnArguments: 0,
			});
			expect(target).toEqual(
				Uint8Array.from([137, 9, 97, 3, 0, 99, 255, 97, 2, 123, 22]),
			);
		});
		it('oid no call or return oid', () => {
			const builder = createBuilder();
			expect(() =>
				builder.oid()()((build: BuilderCore) => {
					build.debug();
				}),
		...
		it('only call oid defined, oid prohibits build commmands', () => {
			...
			expect(len).toBe(9);
			expect(advance).toEqual({
				offsetForArguments: 9,
				offsetForReturnArguments: 0,
			});
			expect(target).toEqual(
				Uint8Array.from([137, 7, 97, 3, 1, 2, 0, 96, 0x50]),
			);
		});
		it('call oids will resolve to NaN', () => {
			const builder = createBuilder();
			expect(() =>
				builder.oid('Poi' as UpToThreeDigitNumberString, '2', '0')()(
					(build: BuilderCore) => {
						build.skip();
					},
				),
	...
	it('clear()', () => {
		...
		expect(builder.debug()).toEqual([
			{ value: 123, valueType: 32 },
			{ value: 456, valueType: 32 },
		]);
		builder.clear();
		expect(builder.debug()).toEqual([]);
	});
