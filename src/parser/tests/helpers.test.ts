...
describe.concurrent('parser/helper', () => {
	...
	describe('ubyte', () => {
		it('fidelity', () => {
			...
			*/
			...
			expect(adv2).toEqual({
				...
				offsetForReturnArguments: 7,
			});

			const retUbytes = getUbyte(data, adv2);
			expect(adv2).toEqual({
				offsetForArguments: 0,
				offsetForReturnArguments: 8,
			});

			const int1 = getInt(data, adv2);
			expect(adv2).toEqual({
				offsetForArguments: 0,
				offsetForReturnArguments: 10,
			});

			const int2 = getInt(data, adv2);
			expect(adv2).toEqual({
				offsetForArguments: 0,
				offsetForReturnArguments: 12,
			});
		...
		it('ubyte misalignment', () => {
			...
			expect(() => getUbyte(dataMalformed, advance)).toThrow(
				'Size of Ubyte type exceeds un parsed memory',
			);
		});
	});
});
